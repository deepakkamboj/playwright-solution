 ...
 */
const getLogger = (): string => (global as any).__TEST_CONTEXT__.logFileFullPath; // eslint-disable-line @typescript-eslint/no-explicit-any
...
export function isHeadlessRun(): boolean {
  return getEnvironmentVariable(EnvironmentVariable.HEADLESS, str => (str ? Number(str) : 0)) === 1;
}
...
export function isVideoRecordingEnabled(): boolean {
  return getEnvironmentVariable(EnvironmentVariable.RECORD_VIDEO, str => (str ? Number(str) : 0)) === 1;
}
...
export function isLoggingEnabled(): boolean {
  return getEnvironmentVariable(EnvironmentVariable.ENABLE_LOGGING, str => (str ? Number(str) : 0)) === 1;
}

function isTraceDebugEnabled(): boolean {
  return (
    getEnvironmentVariable(EnvironmentVariable.TRACE_DEBUG_ENABLED, str => (str ? Number(str) : 0)) === 1
  );
}
...
async function getIdentifier(page: Page, element: ElementHandle<Node>): Promise<string> {
  const data: Array<any> = await page.evaluate(e => {
    const el = e as HTMLElement;
    return [el.getAttribute('data-automation-id'), el.getAttribute('id'), el.getAttribute('class')];
  }, element);
  const attribute = data.find(s => s);
  if (!attribute) {
    throw new Error(`Could not find data-automation-id, id, or class attribute on clicked element`);
  }
  return attribute;
}
